...
def _render_editable_composition(food_name: str, food_data: dict, scope: str = "") -> dict:
    edit_cols = {
        "PB (%)": "PB",
        "EE (%)": "EE",
        "Cenizas (%)": "Ash",
    ...
    scope_key = "".join(c if c.isalnum() else "_" for c in str(scope or "global"))
    session_key = f"comp_data_{scope_key}_{safe_key}"
    ...
    with st.expander("✏️ Editar valores", expanded=False):
        ...
        edited = st.data_editor(
            ...
            hide_index=True,
            key=f"comp_editor_{scope_key}_{safe_key}",
            column_config={
...
def show_food_analysis():
    ...
    with col_info:
        plural = {"perro": "perros", "gato": "gatos"}.get(str(especie_norm).lower(), "")
    ...
    edited_food_data = _render_editable_composition(food_name, food_data, scope=scope_key)
    ...
    safe_key_base = "".join(c if c.isalnum() else "_" for c in food_name)
    safe_key = f"{scope_key}_{safe_key_base}"
